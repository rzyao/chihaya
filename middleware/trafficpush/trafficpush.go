...
func NewHook(cfg Config) (middleware.Hook, error) {
    // 校验与默认值填充
    if cfg.StreamKey == "" { cfg.StreamKey = "tracker:traffic" }
    if cfg.LastKeyPrefix == "" { cfg.LastKeyPrefix = "tracker:last" }
...
func (h *hook) HandleAnnounce(ctx context.Context, req *bittorrent.AnnounceRequest, resp *bittorrent.AnnounceResponse) (context.Context, error) {
    ...
    if uploaded >= lastUp { du = uploaded - lastUp } else { du = uploaded }
    if downloaded >= lastDown { dd = downloaded - lastDown } else { dd = downloaded }
    ...
    if du == 0 && dd == 0 && req.Event == bittorrent.None { return ctx, nil }

    args := redis.Args{}.Add("XADD").Add(h.cfg.StreamKey).Add("*")
    // 6) 计算时长与响应建议间隔（秒）
    dt := int64(0)
    if lastTs > 0 { dt = nowTs - lastTs }
    ...
    for i := 0; i < h.cfg.RetryCount; i++ {
        if _, err = conn.Do("XADD", h.cfg.StreamKey, "*",
            "passkey", passkey,
            "infohash", ih,
            "peer_id", peerID,
            "port", port,
    ...
    if err != nil { log.Error("traffic push: XADD failed", log.Err(err)) }

    return ctx, nil
}

func (h *hook) HandleScrape(ctx context.Context, _ *bittorrent.ScrapeRequest, _ *bittorrent.ScrapeResponse) (context.Context, error) { return ctx, nil }
