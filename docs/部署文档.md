...
- Helm（Kubernetes）
  - 进入 chart：`dist/helm/chihaya`
  - 安装：`helm install tracker ./chihaya --namespace tracker --create-namespace`
  - 按需通过 `values.yaml` 或 ConfigMap 注入你的 `example_config.yaml` 内容

## 部署与运行
- 直接运行（Windows）
  - `.in\chihaya.exe --config C:\path\to\chihaya.yaml`
- 直接运行（Linux）
  - `./bin/chihaya --config /etc/chihaya.yaml`
